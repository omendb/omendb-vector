...
class TestFlushReopen:
    ...
    def test_vectors_survive_reopen(self):
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            db = omendb.open(path)
            c = db.collection("test", config=omendb.CollectionConfig(dim=128))
            vecs = {}
            for i in range(100):
                v = _random_vec()
                vecs[f"d-{i}"] = v
    ...
    def test_relationships_survive_reopen(self):
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            db = omendb.open(path)
            c = db.collection(
                "test", config=omendb.CollectionConfig(dim=128, graph=True)
            )
            c.set("a", vector=_random_vec())
            c.set("b", vector=_random_vec())
    ...
    def test_cosine_metric_survives_reopen(self):
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            db = omendb.open(path)
            c = db.collection(
                "test", config=omendb.CollectionConfig(dim=128, metric="cosine")
            )
            v = _random_vec()
            c.set("a", vector=v)
            c.flush()
